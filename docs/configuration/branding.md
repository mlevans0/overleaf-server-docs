...
The navigation bar title can be customized with the `SHARELATEX_NAV_TITLE` environment variable, this text is used in the top left corner of navigation if no logo is set.
...
### Logo
You can add a custom logo rather than using text by setting the environment variable `SHARELATEX_HEADER_IMAGE_URL`. This value should point to an externally hosted image file.

```bash
# add to config/variables.env
SHARELATEX_HEADER_IMAGE_URL=https://mysite.somewhere.com/img/logo.png
```
...
Extra navigation items can be added to the navigation header by setting the `SHARELATEX_HEADER_EXTRAS` environment variable to a JSON array of objects. For example:
    ...
    SHARELATEX_HEADER_EXTRAS=[{"text":"Some link","url":"http://example.com/somelink","class":"subdued","only_when_logged_out":true},{"text":"Help","class":"subdued","dropdown":[{"text":"Documentation","url":"/learn"},{"text":"Contact Admin","url":"http://example.com/contact"}]}]
